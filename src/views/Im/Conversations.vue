                  ...
                  <div class="conversation-bottom">
                    <div class="conversation-content" v-if="conversation.lastMessage.recalled">
                      <div v-if="conversation.type === 'private'">
                        {{ conversation.lastMessage.senderId === currentUser.id ? '你' : `"${conversation.data.name}"`
                        }}撤回了一条消息
                      </div>
                      <div v-if="conversation.type === 'group'">
                        {{ conversation.lastMessage.senderId === currentUser.id ? '你' :
                          `"${conversation.lastMessage.senderData.name}"` }}撤回了一条消息
                      </div>
                    </div>
                    <div class="conversation-content" v-else>
                      ...
                      <div v-else>
                        {{ conversation.lastMessage.senderId === currentUser.id ? '我' :
...
export default {
  ...
  created() {
    ...
    console.log('fffff', this.currentUser)
...
.menu-header[data-v-fae5bece] {
  margin: 20px auto;
}
.user-avatar[data-v-fae5bece] {
  width: 40px;
  height: 40px;
  border-radius: 10px;
  cursor: pointer;
}
.user-avatar:hover + .user-profile[data-v-fae5bece] {
  display: block;
}
.user-profile[data-v-fae5bece] {
  display: none;
  color: #FFFFFF;
  position: absolute;
  top: 0;
...
.user-profile-main[data-v-fae5bece] {
  border: 1px solid #EBEEF5;
  background-color: #FFF;
  color: #303133;
  border-radius: 4px;
}
.user-profile-header[data-v-fae5bece] {
  padding: 18px 20px;
...
.user-profile-header img[data-v-fae5bece] {
  width: 45px;
  height: 45px;
}
.user-profile-info[data-v-fae5bece] {
  display: flex;
  padding: 10px 20px;
  font-size: 14px;
  color: #666666;
...
.user-profile-info-title[data-v-fae5bece] {
  width: 70px;
}
.menu-box[data-v-fae5bece] {
  padding: 40px 0;
  flex: 1;
  display: flex;
  flex-direction: column;
...
.router-link-active i[data-v-fae5bece] {
  color: #d02129 !important;
}
.iconfont[data-v-fae5bece] {
  padding: 15px;
  font-size: 28px;
  color: #606266;
  cursor: pointer;
}
.iconfont[data-v-fae5bece]:active {
  color: #d02129;
}
