<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.keyword" placeholder="请输入关键字" style="width: 200px; margin-right: 10px;"
        class="filter-item" @keyup.enter.native="handleFilter" />
      <el-select v-model="type_val" placeholder="请选择分类" class="filter-item">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" stripe fit highlight-current-row style="width: 100%;"
      ...
      <el-table-column label="所属地区" min-width="80" prop="limtmoney">
        <template slot-scope="scope">
          {{ scope.row.limit_user_amount }}
        </template>
      </el-table-column>
      <el-table-column label="浏览量" min-width="80" prop="limtmoney">
        <template slot-scope="scope">
          {{ scope.row.limit_user_amount }}
        </template>
      </el-table-column>
      <el-table-column label="是否热门" min-width="80" prop="limtmoney">
        <template slot-scope="scope">
          <span style="color:#53d6c4;  cursor: pointer;" v-if="scope.row.status == 1">是</span>
          <span style="color:#ff643a; cursor: pointer;" v-else>否</span>
        </template>
      </el-table-column>
      <el-table-column label="是否高端" min-width="80" prop="limtmoney">
        <template slot-scope="scope">
          <span style="color:#53d6c4;  cursor: pointer;" v-if="scope.row.status == 1">是</span>
          <span style="color:#ff643a; cursor: pointer;" v-else>否</span>
        </template>
      </el-table-column>
      <el-table-column label="是否空降" min-width="80" prop="limtmoney">
        <template slot-scope="scope">
          <span style="color:#53d6c4;  cursor: pointer;" v-if="scope.row.status == 1">是</span>
          <span style="color:#ff643a; cursor: pointer;" v-else>否</span>
        </template>
      </el-table-column>
      <el-table-column label="所属标签" prop="statue" min-width="240">
        <template slot-scope="scope">
          <span style="background:#ff643a;color:#fff; padding: 3px; margin: 0 5px; cursor: pointer;">美少女</span>
          <span style="background:#ff643a;color:#fff; padding: 3px;margin: 0 5px;cursor: pointer;">人妻</span>
          <span style="background:#ff643a;color:#fff; padding: 3px;margin: 0 5px;cursor: pointer;">热舞</span>
        </template>
      </el-table-column>
      <el-table-column label="市场价" min-width="100" prop="limtmoney">
        <template slot-scope="scope">
          {{ scope.row.limit_user_amount }}
        </template>
      </el-table-column>
      <el-table-column label="优惠价" min-width="100" prop="limtmoney">
        <template slot-scope="scope">
          {{ scope.row.limit_user_amount }}
        </template>
      </el-table-column>
      <el-table-column label="2次价" min-width="100" prop="limtmoney">
        <template slot-scope="scope">
          {{ scope.row.limit_user_amount }}
        </template>
      </el-table-column>
      <el-table-column label="包夜价" min-width="100" prop="limtmoney">
        <template slot-scope="scope">
          {{ scope.row.limit_user_amount }}
        </template>
      </el-table-column>
        ...
        <div style="width: 500px;">
          ...
          <el-form-item label="所属地区">
            <el-checkbox-group v-model="checkList">
              <el-checkbox label="1">全国</el-checkbox>
              <el-checkbox label="2">上海</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
          <el-form-item label="浏览量" prop="limit_amount">
          ...
          <el-form-item label="所属标签" prop="name">
            <el-input v-model="temp.name" />
            <span>多个标签使用英文,隔开；如：美少女,人妻,热舞</span>
          </el-form-item>
          <el-form-item label="市场价" prop="limit_amount">
            <el-input v-model="temp.limit_amount" type="number" />
          </el-form-item>
          <el-form-item label="优惠价" prop="limit_amount">
            <el-input v-model="temp.limit_amount" type="number" />
          </el-form-item>
          <el-form-item label="2次价" prop="limit_amount">
            <el-input v-model="temp.limit_amount" type="number" />
          </el-form-item>
          <el-form-item label="包夜价" prop="limit_amount">
            <el-input v-model="temp.limit_amount" type="number" />
          </el-form-item>
        </div>
...
import { fetchxiaoroomlist, addxiaoroom, delxiaoroom, build_jiangqi, fetchbigroomlist } from '@/api/room_sys'
...
export default {
  ...
  created() {
    this.listQuery.pid = 1//Number(this.$route.query.id)
  ...
  methods: {
    ...
    get_type_List() {
      ...
      fetchbigroomlist(listQuery).then((response) => {
